...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = self.mu_layer(hidden_features)
        logvar = self.logvar_layer(hidden_features)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x_flattened = x.reshape(x.shape[0], x.shape[2] * x.shape[3])
        x_cat = torch.cat((x_flattened, c), dim = 1)
        hidden_features = self.encoder(x_cat)
        mu = self.mu_layer(hidden_features)
        logvar = self.logvar_layer(hidden_features)
        z = reparametrize(mu, logvar)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    left_term = nn.functional.binary_cross_entropy(x_hat, x, reduction = 'sum') 
    var = torch.exp(logvar)
    square_mean = mu.pow(2)
    right_term= torch.sum(- 1 / 2 * (1 + logvar - square_mean - var))
    loss = (left_term + right_term) / x.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
