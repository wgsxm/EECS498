...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for index, coodinate in enumerate(indices):
      x[coodinate] = values[index]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(start, stop + 1):
      if i % 10 == 0:
        temp.append(i)
    x = torch.tensor(temp, dtype = torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    for i, j in zip(x, y):
      result_list.append(torch.matmul(i, j))
    z = torch.stack(result_list)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
