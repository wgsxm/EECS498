...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = torch.sum((x_train_flattened[i] - x_test_flattened[j]) ** 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flattened = x_train.reshape((num_train, -1))
    x_test_flattened = x_test.reshape((num_test, -1))
    dists = torch.sum(x_train_flattened ** 2, dim=1, keepdim=True) - 2 * x_train_flattened @ x_test_flattened.t() + torch.sum(x_test_flattened ** 2, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, min_indices = torch.topk(dists[:, i], k, largest=False)
        min_labels = y_train[min_indices]
        y_pred[i] = torch.min(torch.mode(min_labels).values)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
